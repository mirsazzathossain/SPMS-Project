...
'''
SELECT p.ploNum as plonum, count(*)
FROM app_registration_t r, 
    app_assessment_t a, 
    app_evaluation_t e, 
    app_co_t co, 
...
WHERE r.registrationID = e.registration_id 
    ...
    and a.co_id=co.coID 
    and co.plo_id = p.ploID 
    and r.semester IN ('Spring 2020')
    and p.program_id = '1'
GROUP BY p.ploNum;

SELECT p.plonum, COUNT(*)
FROM progacvstotal p
WHERE p.percentage >= 40
GROUP BY p.plonum


DROP VIEW IF EXISTS progacvstotal;
CREATE VIEW progacvstotal AS
SELECT p.ploNum as plonum, 100*(e.obtainedMarks/a.totalMarks) as percentage
FROM app_registration_t r, 
    app_assessment_t a, 
    app_evaluation_t e, 
    app_co_t co, 
...
WHERE r.registrationID = e.registration_id 
    ...
    and r.semester IN ('Spring 2020')
    and p.program_id = prog.programID
    and prog.programName = 'B.Sc. in CSE';
